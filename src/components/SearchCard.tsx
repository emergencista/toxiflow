    ...
    >
      <div>
        <input
          ...
          className="w-full rounded-2xl border border-slate-200 bg-white/95 px-4 py-4 text-base text-slate-950 outline-none ring-0 transition placeholder:text-slate-400 focus:border-red-400 focus:shadow-[0_0_0_4px_rgba(248,113,113,0.12)]"
        />
              ...
              <div className="max-h-72 overflow-y-auto">
                {results.map((drug) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-3 border-b border-slate-100 px-4 py-3.5 text-left transition last:border-b-0 hover:bg-slate-50"
                  >
                    <span>
                      <span className="block font-medium text-slate-900">{drug.name}</span>
      ...
      {selectedDrug ? (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-base font-semibold text-slate-950">{selectedDrug.name}</h3>
